...
export default function ExamsPage() {
  ...
  const fetchExams = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/v1/admin/universities/${university.id}/colleges/${college.id}/exams?${queryParams}`,
        {
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingExam
        ? `/api/v1/admin/universities/${university.id}/colleges/${college.id}/exams/${editingExam.id}`
        : `/api/v1/admin/universities/${university.id}/colleges/${college.id}/exams`;
  ...
  const handleDelete = async (examId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/v1/admin/universities/${university.id}/colleges/${college.id}/exams/${examId}`,
        {
          method: 'DELETE',
          headers: {
            'X-Request-ID': `req_${Date.now()}`,
